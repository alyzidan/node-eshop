...
router.get('/', async (req, res) => {
	...
	if (!categories) return res.status(500).json({ success: false, message: 'cannot find any categories' })
...
router.put('/:id', async (req, res) => {
	...
	if (!category) return res.status(400).json({ message: 'operation could not be completed' })
...
router.get('/:id', async (req, res) => {
	...
	if (!category) {
		return res.status(500).json({
			message: 'error finding given category , or no such category exists',
		})
	}
